...
function Dashboard() {
  ...
  const fetchDataTotalEur = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`${url}/Movements/totaleur/${formattedDate}/`, {
        headers: {
          Authorization: `Bearer ${accessAdminToken.access_token}`,
        },
      });
  ...
  const fetchDataTotalGbp = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`${url}/Movements/totalgbp/${formattedDate}/`, {
        headers: {
          Authorization: `Bearer ${accessAdminToken.access_token}`,
        },
      });
  ...
  const fetchDataTotalUsd = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`${url}/Movements/totalusd/${formattedDate}/`, {
        headers: {
          Authorization: `Bearer ${accessAdminToken.access_token}`,
        },
      });
  ...
  const fetchDataTotalBs = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`${url}/Movements/totalbs/${formattedDate}/`, {
        headers: {
          Authorization: `Bearer ${accessAdminToken.access_token}`,
        },
      });
  ...
  useEffect(() => {
    ...
    fetchDataAdmin();
    fetchDataCurrency();
  }, [fetchDataAccBs, fetchDataAccUsd, fetchDataUsers, fetchData, fetchDataTotalEur, fetchDataTotalGbp, fetchDataTotalUsd, fetchDataTotalBs, fetchDataAdmin, fetchDataCurrency]);
  ...
  const handleSubmitSendVerify = async event => {
    ...
    if (select.mov_typeOutflow === 'Efectivo') {
      ...
      formData.append('mov_amount', (select.mov_currency === payment ? parseFloat(select.mov_amount) : select.mov_currency === 'USD' && select.mov_currency !== payment ? parseFloat(select.mov_amount) * currencyPrice.cur_UsdToBs : select.mov_currency === 'EUR' ? parseFloat(select.mov_amount) * currencyPrice.cur_EurToBs : select.mov_currency === 'GBP' ? parseFloat(select.mov_amount) * currencyPrice.cur_GbpToBs : parseFloat(select.mov_amount)));
      formData.append('mov_date', formattedDate);
    }
    ...
    try {
      ...
      await axios.get(
        `${url}/Movements/verif/${select.mov_id}`,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${accessAdminToken.access_token}`,
      ...
      await axios.post(
        ...
        null, // Agrega null o el cuerpo de la solicitud, si es necesario.
        {
          headers: {
            Authorization: `Bearer ${accessAdminToken.access_token}`,
          },
  ...
  const handleSubmitCancel = async event => {
    ...
    try {
      await axios.get(
        `${url}/Movements/reject/${select.mov_id}`,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${accessAdminToken.access_token}`,
          ...
          {admin.adm_role === 'A' ? (
            <>
                ...
                <ModalBody>
                  ...
                  {select &&
                    <FormGroup>
                      <Label for="amount">
                        {
                          (select && select.mov_typeOutflow === 'Cuenta Bancaria') || (select && select.mov_typeOutflow === 'Transferencias por dólares') || (select && select.mov_typeOutflow === 'Pago Movil') ?
                            'Monto a transferir' :
                            'Monto a Entregar'
                        }
                      </Label>
                      <Input
                        ...
                        disabled
                        defaultValue={
                          select.mov_currency === 'EUR' && select.mov_typeOutflow === 'Transferencias por dólares' ? ((parseFloat(select.mov_amount)) + (parseFloat(select.mov_amount)) * (currencyPrice.cur_EurToUsd / 100)) :
                            select.mov_currency === 'GBP' && select.mov_typeOutflow === 'Transferencias por dólares' ? ((parseFloat(select.mov_amount)) + (parseFloat(select.mov_amount)) * (currencyPrice.cur_GbpToUsd / 100)) :
                              select.mov_currency === 'USD' && select.mov_typeOutflow !== 'Efectivo' && select.mov_typeOutflow !== 'Transferencias por dólares' ? parseFloat(select.mov_amount) * currencyPrice.cur_UsdToBs :
                                select.mov_currency === 'EUR' && select.mov_typeOutflow !== 'Efectivo' && select.mov_typeOutflow !== 'Transferencias por dólares' ? parseFloat(select.mov_amount) * currencyPrice.cur_EurToBs :
                                  select.mov_currency === 'GBP' && select.mov_typeOutflow !== 'Efectivo' && select.mov_typeOutflow !== 'Transferencias por dólares' ? parseFloat(select.mov_amount) * currencyPrice.cur_GbpToBs :
                                    select.mov_amount}
                      />
                    </FormGroup>}
                  {
                ...
                <ModalFooter>
                  {!showCommentBox ? (
                    <>
                      ...
                      <Button
                        color="success"
                        disabled={((select && select.mov_typeOutflow === 'Cuenta Bancaria') || (select && select.mov_typeOutflow === 'Transferencias por dólares') || (select && select.mov_typeOutflow === 'Pago Movil')) && (payment === '' || bankOptionPay === '' || mov_img === '')}
              ...
              <Modal isOpen={modalImageMov} size='lg' centered toggle={toggleImageMov}>
                <ModalHeader toggle={toggleImageMov}>Verificación de imagen</ModalHeader>
                <ModalBody>
                  {select && select.mov_img.toLowerCase().includes('.pdf') && (
                    <a href={`${url}/Movements/image/${select.mov_img}`} target='_blank' without rel="noreferrer" download>
                      Descargar PDF
                    </a>
                  )} 
                  {select && !select.mov_img.toLowerCase().includes('.pdf') && (
            ...
            : admin.adm_role === 'C' ? (
              <>
                ...
                <div className="DashboardBody">
                    ...
                    <Row>
                      <Col>
                        ...
                        <table className="transaction-table">
                          ...
                          <tbody>
                            {movements
                              .filter((mov) => (mov.mov_status === 'E' && mov.mov_type !== 'Retiro'))
                              .map((move) => (
                                <tr key={move.mov_id}>
                                  <td>{move.User.use_name} {move.User.use_lastName}</td>
                                  <td>{move.mov_currency}</td>
                                  ...
                                  <td>
                                    <button className="details-button" onClick={() => toggle(move)}>
                                      Ver detalles
                                    </button>
                                  </td>
                                </tr>
                  ...
                  <Button color="secondary" onClick={toggleModalIngreso} disabled={isLoadingVerify}>
                    Volver
                  </Button>
                </ModalFooter>
                    </Modal>
                ...
                <>
                  ...
                  <div className="DashboardBody">
                    <Container>
                      ...
                      <Row>
                        <Col>
                          <table success bordered hover responsive striped className="transaction-table ">
                            <thead>
                              <tr>
                                <th>Usuario</th>
                                <th>Moneda</th>
                          ...
                          {select &&
                            <FormGroup>
                              ...
                              <Input
                                ...
                                disabled
                                defaultValue={
                                  select && select.mov_currency === 'USD' && select.mov_typeOutflow !== 'Efectivo' ?
                                    parseFloat(select.mov_amount) * parseFloat(currencyPrice.cur_UsdToBs)
                                    : select.mov_currency === 'EUR' && select.mov_typeOutflow !== 'Efectivo' ?
                                      parseFloat(select.mov_amount) * parseFloat(currencyPrice.cur_EurToBs)
                                      : select.mov_currency === 'GBP' && select.mov_typeOutflow !== 'Efectivo' ?
                                        parseFloat(select.mov_amount) * parseFloat(currencyPrice.cur_GbpToBs)
                                        : select.mov_amount} />
                            </FormGroup>}
                          {
                            ((select && select.mov_typeOutflow === 'Cuenta Bancaria') || (select && select.mov_typeOutflow === 'Transferencias por dólares') || (select && select.mov_typeOutflow === 'Pago Movil')) &&
